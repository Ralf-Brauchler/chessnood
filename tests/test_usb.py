...
# --- converging-cross LED animation (board.led_animation) -------------------

def test_cross_frames_converge_on_target():
    frames = usb.cross_frames(chess.E4)
    # arms at distance 3,2,1 then the target alone
    assert len(frames) == usb.ANIM_REACH + 1
    assert frames[-1] == usb.encode_leds([chess.E4])       # settles on exactly the target
    # the target is lit in every single frame
    target_bit = usb.encode_leds([chess.E4])
    for fr in frames:
        for i in range(2, 10):
            assert (fr[i] & target_bit[i]) == target_bit[i]


def test_cross_frames_clip_at_the_edge():
    # a corner has only two on-board arms; must not raise or wrap around
    frames = usb.cross_frames(chess.A1)
    assert frames[-1] == usb.encode_leds([chess.A1])
    assert len(frames) == usb.ANIM_REACH + 1


def test_set_leds_queues_animation_only_for_single_target():
    import asyncio
    board = usb.UsbBoard(animate=True)                     # no device attached
    asyncio.run(board.set_leds([chess.E4]))
    assert board._anim_frames == usb.cross_frames(chess.E4)
    # a different single target re-arms the animation
    asyncio.run(board.set_leds([chess.D5]))
    assert board._anim_frames == usb.cross_frames(chess.D5)
    # multi-square (e.g. castling) does not animate
    asyncio.run(board.set_leds([chess.E1, chess.G1]))
    assert board._anim_frames == []


def test_animation_off_by_default_writes_directly():
    import asyncio
    board = usb.UsbBoard()                                 # animate defaults off
    asyncio.run(board.set_leds([chess.E4]))
    assert board._anim_frames == []                        # no animation queued
